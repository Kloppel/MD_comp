...
universe = mda.Universe("1eru.psf", "1eru_eq.dcd")
...
# Access velocities for the current frame
...
# Schleife über alle Frames in der Trajektorie
for ts in u.trajectory:
    velocities = ts.velocities
    speed = np.linalg.norm(velocities, axis=1)
    all_speeds.extend(speed)  # Geschwindigkeiten sammeln

# Histogramm über die gesamte Trajektorie
plt.hist(all_speeds, bins=50, density=True)
plt.xlabel('Geschwindigkeit (Å/fs)')
plt.ylabel('Wahrscheinlichkeit')
plt.title('Geschwindigkeitsverteilung (gesamte Trajektorie)')
plt.show()
