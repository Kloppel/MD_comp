...
dcdReporter = DCDReporter('../outfile/4pti_noheat.dcd', 100)
dataReporter = StateDataReporter('../outfile/4pti_noheat.txt', 100, totalSteps=equilibrationSteps,
                                 step=True, speed=True, potentialEnergy=True, kineticEnergy=True, totalEnergy=True, volume=True, temperature=True, separator='\t')
checkpointReporter = CheckpointReporter('../outfile/4pti_noheat.chk', 100)
...
checkpoint_file = '../outfile/4pti_NVT.chk'
...
# Save the equilibrated state to a checkpoint file
checkpoint_file = '../outfile/4pti_noheat.chk'
simulation.saveCheckpoint(checkpoint_file)

state = simulation.context.getState(getPositions=True)

# Speichern der PDB-Datei
with open('../struct/4pti_noheat.pdb', 'w') as output:
